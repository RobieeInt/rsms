<div class="space-y-6">
    ...
    <div class="flex items-start justify-between">
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-3">
                <div class="h-12 w-12 rounded-xl bg-stone-100 dark:bg-stone-900/30 flex items-center justify-center text-stone-700 dark:text-stone-300 font-bold text-lg">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('clients.edit', $client) }}" class="inline-flex items-center gap-2 rounded-lg bg-stone-600 px-3 py-2 text-sm font-medium text-white hover:bg-stone-700 transition-colors">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
            <p class="text-sm text-gray-500 dark:text-gray-400">Temuan Terbuka</p>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
            <p class="text-sm text-gray-500 dark:text-gray-400">Invoice Belum Bayar</p>
    ...
    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
        <div class="border-b border-gray-200 dark:border-gray-700">
            <nav class="flex gap-1 px-4 pt-4" role="tablist">
                @foreach (['assets' => 'Assets', 'schedules' => 'Schedules', 'findings' => 'Findings', 'invoices' => 'Invoices'] as $tab => $label)
                    <button wire:click="setTab('{{ $tab }}')" class="px-4 py-2.5 text-sm font-medium rounded-t-lg transition-colors {{ $activeTab === $tab ? 'bg-stone-50 dark:bg-stone-900/20 text-stone-700 dark:text-stone-300 border-b-2 border-stone-600' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' }}">
                        {{ $label }}
                    </button>
                @endforeach
            </nav>
        ...
        <div class="p-4">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead>
                                <tr>
                                    ...
                                    <th class="py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Aksi</th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($assets as $asset)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td class="py-3 text-right">
                                            <a href="{{ route('assets.show', $asset) }}" class="text-stone-600 dark:text-stone-400 text-xs hover:underline">View</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        @foreach ($schedules as $schedule)
                            <div class="flex items-center justify-between rounded-lg border border-gray-100 dark:border-gray-700 p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <a href="{{ route('schedules.show', $schedule) }}" class="text-stone-600 dark:text-stone-400 text-xs hover:underline">View</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        @foreach ($findings as $finding)
                            <div class="flex items-center justify-between rounded-lg border border-gray-100 dark:border-gray-700 p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <a href="{{ route('findings.show', $finding) }}" class="text-stone-600 dark:text-stone-400 text-xs hover:underline">View</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @if ($activeTab === 'invoices')
                ...
                <div class="mb-4 flex items-center justify-between rounded-xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900/20 px-4 py-3">
                    <div>
                        <p class="text-sm font-medium text-stone-900 dark:text-stone-200">Monthly Retainer: <span class="font-bold">Rp {{ number_format($client->monthly_retainer_fee, 0, ',', '.') }}</span></p>
                        <p class="text-xs text-stone-600 dark:text-stone-400">Generate invoice for {{ now()->format('F Y') }}</p>
                    </div>
                    <button wire:click="generateRetainerInvoice" wire:loading.attr="disabled" class="inline-flex items-center gap-2 rounded-lg bg-stone-600 px-3 py-2 text-sm font-medium text-white hover:bg-stone-700 disabled:opacity-60 transition-colors">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead>
                                <tr>
                                    ...
                                    <th class="py-3 text-right text-xs font-medium text-gray-500 uppercase">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach ($invoices as $invoice)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td class="py-3 text-right"><a href="{{ route('invoices.show', $invoice) }}" class="text-stone-600 dark:text-stone-400 text-xs hover:underline">View</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
